"""Evaluate LongiSeg 5-fold predictions produced with checkpoint_best.pth.

Layout assumed:
    /mnt/hdd2/task2/longiseg/predict_results/mask_results_f{F}_best/{P}_{xxxxx}.png

Ground truth:
    /mnt/hdd2/task2/nnunet/Dataset007_task2_Ts_{P}/labelsTs/{P}_{xxxxx}.png

Metrics: per-image per-class IoU / Dice / HD95 / Precision / Recall, with the
"label-only" valid-class convention (same as recompute_metrics_clean.py).

Outputs:
    predict_results/clean_eval_best/per_image_f{F}_best.csv   (full rows)
    predict_results/5fold_best_eval.csv                       (5-fold summary
                                                               in the same
                                                               columns as
                                                               longiseg_5fold.csv)
"""
...
SUMMARY_CSV = os.path.join(PRED_BASE, "5fold_best_eval.csv")
os.makedirs(OUT_DIR, exist_ok=True)

CROP = (289, 0, 1631, 1004)  # left, upper, right, lower — same as recompute_metrics_clean.py
...
logger = logging.getLogger("longiseg_best")
...
def process_one(args):
    ...
    label_path = os.path.join(LABEL_BASE, f"Dataset007_task2_Ts_{patient}",
                              "labelsTs", f"{name}.png")
...
def process_fold(fold):
    pred_dir = os.path.join(PRED_BASE, f"mask_results_f{fold}_best")
